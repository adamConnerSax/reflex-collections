...
import           Reflex.Collections.Diffable     (Diffable (..), HasEmpty (..),
                                                  toDiff)
...
import           Reflex.Collections.Sequenceable (PatchSequenceable (..),
                                                  ReflexSequenceable (..))
import           Reflex.Collections.ToPatchType  (ToPatchType (..),
                                                  distributeOverDynPure,
                                                  mergeOver, toSeqType)
...
listHoldWithKeyGeneral :: forall t m f k v a. ( R.Adjustable t m
                                              , R.MonadHold t m
                                              , ToPatchType f k v a
                                              , PatchSequenceable (SeqType f k) (SeqPatchType f k) (SeqTypeKey f k a))
...
listWithKeyGeneral :: forall t m f k v a. ( R.Adjustable t m
                                          ...
                                          , PatchSequenceable (SeqType f k) (SeqPatchType f k) (SeqTypeKey f k a) -- for the listHold
                                          , Diffable f (Diff f k)
                                          , HasEmpty (f v)
                                          , Functor (Diff f k)
                                          , HasFan f v
...
listViewWithKeyGeneral' ::  forall t m f k v a. ( R.Adjustable t m
                                                ...
                                                , PatchSequenceable (SeqType f k) (SeqPatchType f k) (SeqTypeKey f k (R.Event t a))
                                                , PatchSequenceable (SeqType f k) (SeqPatchType f k) (SeqTypeKey f k a)
                                                , ReflexSequenceable ((SeqType f k) (SeqTypeKey f k a))
                                                , HasFan f v
                                                , FanInKey f ~ k)
  => (R.Dynamic t (f v) -> m (f v, R.Event t (Diff f k v)))
...
listWithKeyShallowDiffGeneral initialVals valsChanged mkChild = do
  ...
  sentVals <- R.foldDyn applyDiff (empty :: f ()) $ fmap void valsChanged
...
selectViewListWithKeyGeneral :: forall t m f k v a. ( R.Adjustable t m
                                                    ...
                                                    , PatchSequenceable (SeqType f k) (SeqPatchType f k) (SeqTypeKey f k (R.Event t (k,a))) -- for the listHold
