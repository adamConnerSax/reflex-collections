...
class HasFan (f :: Type -> Type) v where
  ...
  default doFan :: (R.Reflex t, ToPatchType f (FanInKey f) v v) => Proxy v -> R.Event t (f v) -> R.EventSelector t (FanSelKey f v)
...
instance Hashable k => HasFan (HashMap k) v where
